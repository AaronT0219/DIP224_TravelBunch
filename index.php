...
if(isset($_POST['submit'])){
    ...
    $get_acc = $conn->prepare("SELECT id, email FROM user_form WHERE email=? AND password=? UNION SELECT id, email FROM merc_form WHERE email=? AND password=?");
    $get_acc->bind_param("ssss", $email, $pass, $email, $pass);
    $get_acc->execute();
                ...
                <?php
                    if ($_SERVER["REQUEST_METHOD"] === "POST") {
                        ...
                        if ($destination === "langkawi") {
                            if ($transport === "bus") {
                                $Price = (($PriceToLangkawi + $PriceByBus) * $pax);
                                echo $Price;
                            } else {
                                $Price = (($PriceToLangkawi + $PriceByPlane) * $pax);
                                echo $Price;
                            }
                        } elseif ($destination === "ipoh") {
                            if ($transport === "bus") {
                                $Price = (($PriceToIpoh + $PriceByBus) * $pax);
                                echo $Price;
                            } else {
                                $Price = (($PriceToIpoh + $PriceByPlane) * $pax);
                                echo $Price;
                            }
                        } elseif ($destination === "melaka") {
                            if ($transport === "bus") {
                                $Price = (($PriceToMelaka + $PriceByBus) * $pax);
                                echo $Price;
                            } else {
                                $Price = (($PriceToMelaka + $PriceByPlane) * $pax);
                                echo $Price;
                            }
                        } elseif ($destination === "georgetown") {
                            if ($transport === "bus") {
                                $Price = (($PriceToGeorgetown + $PriceByBus) * $pax);
                                echo $Price;
                            } else {
                                $Price = (($PriceToGeorgetown + $PriceByPlane) * $pax);
                                echo $Price;
                            }
                        }
                    }
